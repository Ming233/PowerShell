...
Function Main
{
    ...
    #$PRISM_PROD_Backup_Folder = '\\10.190.18.195\SQLBackups\P-PSM-DBS001$PRDM10A\PRISM_PROD\FULL'
    #$PRISM_STAGING_Backup_Folder = "\\protect.ds\infra\SQL_Backup\P-PSM-DBS001\PRISM_STAGING\FULL\"
    #$PRISM_STAGING_Backup_Folder = '\\10.190.18.195\SQLBackups\P-PSM-DBS001$PRDM10A\PRISM_STAGING\FULL'
    ...
    else
    {
        ...
        else
        {
            ...
            if($restore_confirm -eq "Y")
            {
                ...
                Invoke-Sqlcmd -ServerInstance $script:ServerName -Query $querystring -QueryTimeout 1200 -OutputSqlErrors $true -ErrorAction Inquire

                #Invoke-Sqlcmd -ServerInstance $script:ServerName -Variable db_to_restore="$db_to_restore", file_to_restore_from="$file_to_restore_from" -Query "USE master; RESTORE DATABASE `$(db_to_restore) FROM DISK = `$(file_to_restore_from) WITH RECOVERY, REPLACE; " -QueryTimeout 1200 -OutputSqlErrors $true -ErrorAction Inquire
...
Function Restore_DEV_From_Last_PRISM_Backup
{
    ...
    if($STAGING_Archive_Backup_Folder -eq $null -or $STAGING_Archive_Backup_Folder -eq "")
    {
        RaiseError "Error! A variable named: <STAGING_Archive_Backup_Folder> must be defined and provided with a value in order to do a database restore!" $false
    }
	...
	if($STAGING_Archive_Backup_Folder -gt ""}
		...
		# Each file has _1.bak, _2.bak, etc. at the end. 
		$file_to_restore_from = $file_to_restore_from.Substring(0, $file_to_restore_from.Length - 5) + "*.bak"
		$db_to_restore = "PRISM_STAGING_ARCHIVE"
