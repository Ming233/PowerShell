param([string]$JSON_File = "", $StepsOutput = "", $JSON_File_svnfolder = "", $JSON_File_svnrevision = "", $working_Folder = "", $Environment = "")
...
Function Main
{
    ...
    $email_after_step_completes = "N"
    $email_after_every_step_completes = "N"
    ...
    $PRISM_PROD_Backup_Folder = "\\protect.ds\infra\SQL_Backup\P-PSM-DBS001\PRISM_PROD\FULL\"
    $PRISM_STAGING_Backup_Folder = "\\protect.ds\infra\SQL_Backup\P-PSM-DBS001\PRISM_STAGING\FULL\"
...
Function Replace_Var_Values($varvalue)
{
    ...
    #Write-Host "varvalue is $varvalue"
    ...
    #Write-Host "varvalue AFTER is $varvalue"
...
Function Get_Full_Path_For_File($filename)
{
    ...
    #read-host "at end of get_full_path_for_File >$fullpath<" | out-nULL
...
Function Restore_Database($db_to_restore, $file_to_restore_from)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if($environment -ne "DEV")
            {
                ...
                #$restore_confirm = Read-Host -Prompt "Are you sure you want to restore the database: <$db_to_restore> from the file: <$file_to_restore_from>? (Y/N)"
            ...
            if($restore_confirm -eq "Y")
            {
                ...
                Invoke-Sqlcmd -ServerInstance $ServerName -Variable db_to_restore="$db_to_restore", file_to_restore_from="$file_to_restore_from" -Query "USE master; RESTORE DATABASE `$(db_to_restore) FROM DISK = `$(file_to_restore_from) WITH RECOVERY, REPLACE; " -QueryTimeout 1200 -OutputSqlErrors $true -ErrorAction Inquire
            }
            else
            {
...
Function Restore_DEV_From_Last_PRISM_Backup
{
    ...
    $file_to_restore_from = (gci $PRISM_PROD_Backup_Folder -Filter *.bak | sort LastWriteTime | Select -last 1).FullName
    $db_to_restore = $prism_db

    Restore_Database $db_to_restore $file_to_restore_from

    # Restore PRISM_STAGING backup file to database.
    $file_to_restore_from = (gci $PRISM_STAGING_Backup_Folder -Filter *.bak | sort LastWriteTime | Select -last 1).FullName
    $db_to_restore = $staging_db
...
Function Rename_Database($db_to_rename, $new_db_name, $raise_error_if_rename_not_needed, $confirm_rename)
{
    ...
    else
    {
        ...
        if($rename_confirmed -eq "Y")
        {
            ...
            Invoke-Sqlcmd -ServerInstance $ServerName -Variable db_to_rename="$db_to_rename", new_db_name="$new_db_name", raise_error_if_rename_not_needed="$raise_error_if_rename_not_needed" -InputFile "$rename_database_file" -QueryTimeout 60 -OutputSqlErrors $true -ErrorAction Inquire
        }
        else
        {
...
Function Execute_Verification_SQL($db_to_use, $sql_to_use, $sql_script_to_use, $svnfolder, $svnrevision, $comparison_return_value, $comparison_operator, $data_type, $stop_if_not_matching)
{
    ...
    if($sql_to_use -gt "")
    {
        ...
        $queryresult = Invoke-Sqlcmd -ServerInstance $script:ServerName -Database $script:db_to_use -Query $script:sql_to_use -QueryTimeout 300 -OutputSqlErrors $true -ErrorAction Inquire
    }
    else
    {
        if($svnfolder -gt "" -and $svnrevision -gt "")
        ...
        $queryresult = Invoke-Sqlcmd -ServerInstance $script:ServerName -Database $script:db_to_use -InputFile $sql_script_to_use_WithPath -QueryTimeout 300 -OutputSqlErrors $true -ErrorAction Inquire
    }
...
Function Execute_Powershell_Script($script_command)
{
    ...
    write-host "script_command = $script_command"
    Invoke-Expression $script_command_withPath | write-host
}

Function Move_File($sourcefile, $destinationfile)
{
    If (!(Test-Path $sourcefile))
    {
        RaiseError "The source file $sourcefile is not found!" $true
    }
    else
    {
        if(Test-Path $destinationfile)
        {
            RaiseError "The destination file $destinationfile already exists! Continue to overwrite this file." $true
        }

	    Move-Item $sourcefile $destinationfile
    }
}
        ...
        for ($i=0; $i -le $data.steps.count - 1; $i++)
        {
                ...
                for ($x=0; $x -le $data.steps[$i].variables.count - 1; $x++)
                {
                    ...
                    if($variablename -gt "")
                    {
                        ...
                        if($overridevariable -eq "TRUE" -or (!(Test-Path "variable:script:$variablename")))
                        {
                            ...
                            $usevariablevalue = $data.steps[$i].variables[$x].usevariablevalue

                            if($variablevalue -gt "")
                            {
                                if($usevariablevalue -eq "TRUE")
                                {
                                    $tempvalue = Get-Variable -Scope script -Name $variablevalue -ValueOnly -ErrorAction Ignore
                                    New-Variable -Scope script -Name $variablename -Force -Value $tempvalue
            ...
            elseif(($Script:ScriptEnvironment -eq $Script:Environment) -or ($Script:Environment -eq "ALL"))
            {
                switch($command)
                {
                    ...
                    "restore_database" {
                                            Restore_Database $script:db_to_restore $script:file_to_restore_from
                                            Step_Complete
                                            # blank out variables after function is called.
                    ...
                    "move_file"
                                {
                                    Move_File $script:sourcefile $script:destinationfile
                                    Step_Complete
                                    # blank out variables after function is called.
                                    $script:sourcefile = ""
                    ...
                    "execute_powershell_script"
                                {
                                    Execute_PowerShell_Script $script:script_command
                                    Step_Complete
